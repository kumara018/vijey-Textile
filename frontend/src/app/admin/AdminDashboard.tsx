...
function AdminPageInner({ initialView }: { initialView?: string }) {
  ...
  //   1. the URL PATH   (/admin/orders) — the address bar is the truth
  //   2. the ?tab= query — kept working for every link already in the wild
  //   3. localStorage    — only for a bare /admin visit
  //
  // The path has to outrank the saved tab for the same reason the query
  // string already did: someone who navigates to /admin/returns is asking for
  // returns, not for whatever they were last looking at.
  useEffect(() => {
    const valid: TabKey[] = ['dash','products','orders','cancellations','users','ratings','returns','admins'];

    if (initialView && valid.includes(initialView as TabKey)) {
      setTab(initialView as TabKey);
      localStorage.setItem('admin_tab', initialView);
      return;
    }
        ...
        {TABS.map(({ key, label }) => {
          ...
          return (
            <button
              key={key}
              /**
               * Sets the tab AND the address.
               *
               * The state change is what re-renders; the URL change is what
               * makes the view addressable — bookmarkable, shareable, and
               * correct under the back button. Both happen, in that order, so
               * the view never waits on a navigation to paint.
               *
               * `router.push` rather than `replace`: moving between admin
               * views is real navigation and belongs in history.
               */
              onClick={() => {
                setTab(key as TabKey);
                localStorage.setItem('admin_tab', key);
                router.push(key === 'dash' ? '/admin' : `/admin/${key}`);
                if (key === 'cancellations') loadOrders();
                if (key === 'returns') loadReturns();
              }}
      ...
      {tab === 'returns' && (
        <div>
          <div className="mb-4 flex items-center gap-3">
            <h2 className="font-bold text-gray-800 text-lg">Return & Exchange Requests</h2>
            {returns.length > 0 && (
...
/**
 * The admin dashboard, mounted by every /admin route.
 *
 * Previously this file WAS the route (`admin/page.tsx`) and its eight views
 * were switched by component state, with `?tab=` as the only way to address
 * one. That made the views unlinkable in the ways that matter to someone
 * running a shop: the back button left a view instead of returning to the
 * previous one, a bookmarked view was a query string on a page that might
 * ignore it, and "send me the returns screen" was not a URL.
 *
 * The views are now real routes — /admin/orders, /admin/returns and so on —
 * each mounting this same component with `initialView` set. Splitting the
 * 2,200 lines into five separate implementations was the alternative, and it
 * would have meant five copies of the notification poller, the auth gate and
 * the Delhivery sync logic diverging from each other on a live shop. One
 * component, many addresses, is the safer shape.
 *
 * `?tab=` still works. Existing links and the saved localStorage tab are not
 * broken by this.
 */
export default function AdminDashboard({ initialView }: { initialView?: string }) {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-maroon-100"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-maroon-700" /></div>}>
      <AdminPageInner initialView={initialView} />
    </Suspense>
  );
}
