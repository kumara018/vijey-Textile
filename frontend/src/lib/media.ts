 ...
 *
 * `g_auto` is doing a second job. The opening is a wide band and a portrait
 * photograph has to be cropped to fill it — `g_auto` picks the region worth
 * keeping rather than taking the middle by default, so the crop is less likely
 * to land across the middle of a garment.
 *
