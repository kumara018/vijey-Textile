    ...
    <section aria-labelledby="kept-heading" className="border-t border-ink-edge py-[6vh]">
      <div className="mx-auto w-full max-w-[112rem] px-6 sm:px-10">
        <div className="mb-[8vh] flex flex-wrap items-baseline justify-between gap-6">
          <Reveal>
            <h2 id="kept-heading" className="font-display text-chapter font-light text-paper">
              Recently kept
            </h2>
          </Reveal>
          <Reveal delay={120}>
            <Link
              href="/products"
              className="group inline-flex items-baseline gap-3 border-b border-ink-edge pb-1.5 text-caption uppercase text-paper-muted transition-colors duration-500 hover:border-maroon-300 hover:text-paper focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-maroon-300"
            >
              Everything
          ...
          <div className="grid grid-cols-1 gap-x-10 gap-y-14 sm:grid-cols-2 lg:grid-cols-12" aria-busy="true">
            {RHYTHM.slice(0, 4).map((r, i) => (
              <div key={i} className={`${r.span} ${r.offset}`}>
                <div className="aspect-[4/5] w-full animate-pulse rounded-sm bg-ink-raised" />
                <div className="mt-5 h-4 w-2/3 animate-pulse rounded-sm bg-ink-raised" />
              </div>
            ))}
            <span className="sr-only">Loading recent pieces</span>
          </div>
        ) : items.length === 0 ? (
          <p className="max-w-[44ch] text-lede text-paper-muted">
