...
#   bg-brand-gradient = linear-gradient(135deg, #0f766e 0%, #134e4a 100%)
...
<body style="margin:0;padding:24px 16px;background:#f0fdfa;font-family:Arial,Helvetica,sans-serif;">
      ...
      <tr>
        <td align="center" style="padding:18px 24px;">
          ...
          <p style="margin:0 0 6px;font-size:12px;font-family:Arial,sans-serif;">
            <a href="mailto:{SUPPORT_EMAIL}" style="color:#0f766e;text-decoration:none;">Contact Support</a>
            &nbsp;&middot;&nbsp;
            <a href="{STORE_URL}" style="color:#0f766e;text-decoration:none;">Visit Store</a>
            &nbsp;&middot;&nbsp;
            <a href="{STORE_URL}/orders" style="color:#0f766e;text-decoration:none;">My Orders</a>
          </p>
...
def _btn(text: str, url: str, bg: str = "#0f766e") -> str:
...
def send_welcome_email(email: str, name: str):
    first = name.split()[0]
    html = _wrap(f"""
      <h2 style="color:#0f766e;margin-top:0;font-size:22px;">Welcome to {STORE_NAME}, {first}! 🎉</h2>
      ...
      <table style="width:100%;border-collapse:collapse;margin:16px 0;">
        <tr>
          <td style="padding:12px 8px;text-align:center;background:#f0fdfa;border-radius:8px;width:30%;">
            <div style="font-size:30px;">👗</div>
            <div style="color:#0f766e;font-weight:bold;font-size:13px;margin-top:6px;">Chudithar &amp; Tops</div>
          </td>
          <td style="width:4%;"></td>
          <td style="padding:12px 8px;text-align:center;background:#f0fdfa;border-radius:8px;width:30%;">
            <div style="font-size:30px;">🪭</div>
            <div style="color:#0f766e;font-weight:bold;font-size:13px;margin-top:6px;">Lehenga &amp; Half Sarees</div>
          </td>
          <td style="width:4%;"></td>
          <td style="padding:12px 8px;text-align:center;background:#f0fdfa;border-radius:8px;width:30%;">
            <div style="font-size:30px;">✨</div>
            <div style="color:#0f766e;font-weight:bold;font-size:13px;margin-top:6px;">Party Wears</div>
          </td>
        </tr>
      </table>
      ...
      <p style="color:#888;font-size:13px;line-height:1.6;margin:0;">
        ...
        <a href="mailto:{SUPPORT_EMAIL}" style="color:#0f766e;">{SUPPORT_EMAIL}</a> — we reply within 24 hours.
      </p>
    """)
...
def send_order_confirmation_email(email: str, name: str, order):
    ...
    html = _wrap(f"""
      ...
      <div style="background:#f8f4f0;border-left:4px solid #0f766e;border-radius:4px;padding:16px;margin:20px 0;">
        <p style="margin:0;color:#888;font-size:11px;letter-spacing:1px;text-transform:uppercase;">Order Number</p>
        <p style="margin:6px 0 0;font-size:22px;font-weight:bold;color:#0f766e;letter-spacing:2px;">{order.order_number}</p>
      </div>
      ...
      <div style="background:#f8f4f0;border-radius:8px;padding:16px;margin:16px 0;">
        ...
        <p style="margin:0;color:#1e293b;font-weight:bold;font-size:16px;">Total &nbsp;<strong style="color:#0f766e;font-size:18px;float:right;">₹{order.total:,.0f}</strong></p>
      </div>
...
def send_payment_success_email(email: str, name: str, order):
    ...
    html = _wrap(f"""
      ...
      <p style="color:#444;font-size:14px;line-height:1.6;">
        Hi {first}, your payment of <strong style="color:#0f766e;">₹{order.total:,.0f}</strong>
      ...
      <table style="width:100%;font-size:14px;margin:16px 0;">
        ...
        <tr><td style="color:#888;padding:6px 0;">Amount Paid</td><td style="color:#0f766e;font-weight:bold;text-align:right;">₹{order.total:,.0f}</td></tr>
...
_STATUS_MAP = {
    ...
    "shipped":          ("📦 Your Order is Shipped!",   "#0f766e", "#f0fdfa", "Your order is on its way — expect it soon!"),
...
def send_order_status_email(email: str, name: str, order, new_status: str):
    ...
    title, color, bg, msg = _STATUS_MAP.get(
        new_status, (f"Order Update: {new_status.title()}", "#0f766e", "#f8f4f0", f"Status: {new_status}")
    )
...
def send_review_request_email(email: str, name: str, order):
    first = name.split()[0]
    html = _wrap(f"""
      <h2 style="color:#0f766e;margin-top:0;font-size:22px;">How did we do? ⭐</h2>
      <p style="color:#444;font-size:14px;line-height:1.6;">
...
def send_deletion_scheduled_email(email: str, name: str, delete_at):
    ...
    html = _wrap(f"""
      ...
      <p style="color:#888;font-size:12px;line-height:1.6;margin:0;">
        ...
        <a href="{retrieve_url}" style="color:#0f766e;">log in immediately</a> to secure it,
        or contact <a href="mailto:{SUPPORT_EMAIL}" style="color:#0f766e;">{SUPPORT_EMAIL}</a>.
      </p>
    """)
    ...
    html = _wrap(f"""
      ...
      <div style="background:#f8f4f0;border-radius:10px;padding:16px;margin:20px 0;text-align:center;">
        <p style="margin:0;color:#555;font-size:14px;">
          ...
          <a href="{STORE_URL}" style="color:#0f766e;font-weight:bold;">{STORE_URL}</a>
        </p>
      </div>
...
def send_login_otp_email(email: str, name: str, otp: str):
    first = name.split()[0]
    html = _wrap(f"""
      <h2 style="color:#0f766e;margin-top:0;font-size:22px;">🔐 Your Login OTP</h2>
      <p style="color:#444;font-size:14px;line-height:1.6;">
        Hi {first}, use the OTP below to complete your sign-in to <strong>{STORE_NAME}</strong>.
      </p>
      <div style="background:#f0fdfa;border:2px solid #0f766e;border-radius:12px;padding:28px;margin:24px 0;text-align:center;">
        <p style="margin:0;color:#888;font-size:11px;text-transform:uppercase;letter-spacing:3px;">One-Time Password</p>
        <p style="margin:14px 0 6px;font-size:46px;font-weight:bold;color:#0f766e;letter-spacing:14px;font-family:monospace;">{otp}</p>
...
def send_password_reset_otp_email(email: str, name: str, otp: str):
    first = name.split()[0]
    html = _wrap(f"""
      <h2 style="color:#0f766e;margin-top:0;font-size:22px;">🔑 Password Reset OTP</h2>
      <p style="color:#444;font-size:14px;line-height:1.6;">
...
def _cart_summary_html(cart_items: list) -> str:
    ...
    for item in cart_items:
        ...
        <tr>
          ...
          <td style="padding:10px 8px;border-bottom:1px solid #f0ebe5;text-align:right;font-weight:bold;color:#0f766e;font-size:14px;">₹{subtotal:,.0f}</td>
        </tr>"""
    return f"""
    <table style="width:100%;border-collapse:collapse;margin:16px 0;">
      <thead>
      ...
      <tfoot>
        <tr>
          ...
          <td style="padding:12px 8px;text-align:right;font-weight:bold;color:#0f766e;font-size:16px;">
            ₹{total_price:,.0f}
          </td>
        </tr>
      </tfoot>
...
def send_cart_add_email(email: str, name: str, product_name: str,
    ...
    html = _wrap(f"""
      <h2 style="color:#0f766e;margin-top:0;font-size:22px;">🛒 Added to your cart!</h2>
      <p style="color:#444;font-size:14px;line-height:1.6;">
        Hi {first}, <strong>{product_name}</strong> (×{quantity}) has been added to your cart.
      </p>

      <!-- Added product highlight -->
      <div style="background:#f0fdfa;border:2px solid #f97316;border-radius:12px;padding:20px;margin:20px 0;display:flex;align-items:center;gap:16px;">
        <div style="font-size:52px;flex-shrink:0;">{emoji}</div>
        <div>
          <p style="margin:0;font-size:16px;font-weight:bold;color:#0f766e;">{product_name}</p>
      ...
      <!-- Full cart summary -->
      <p style="color:#0f766e;font-weight:bold;font-size:15px;margin-bottom:4px;">🧺 Your Cart Summary</p>
      {summary_html}
...
def send_cart_remove_email(email: str, name: str, product_name: str,
    ...
    html = _wrap(f"""
      <h2 style="color:#0f766e;margin-top:0;font-size:22px;">🗑️ Item removed from cart</h2>
      <p style="color:#444;font-size:14px;line-height:1.6;">
      ...
      <!-- Remaining cart -->
      {f'<p style="color:#0f766e;font-weight:bold;font-size:15px;margin-bottom:4px;">🧺 Remaining Cart ({remaining} item{"s" if remaining!=1 else ""})</p>' if remaining > 0 else ''}
      {summary_html}

      {_btn("Continue Shopping →", f"{STORE_URL}/products", "#0f766e") if remaining == 0 else _btn("View Cart & Order →", f"{STORE_URL}/cart", "#f97316")}
    """)
...
def send_delivery_otp_email(email: str, name: str, otp: str, order_number: str,
    ...
    html = _wrap(f"""
      <h2 style="color:#0f766e;margin-top:0;font-size:22px;">🚚 Your order is out for delivery!</h2>
      <p style="color:#444;font-size:14px;line-height:1.6;">
      ...
      <div style="background:#f0fdfa;border:2px solid #0f766e;border-radius:12px;padding:28px;margin:24px 0;text-align:center;">
        ...
        <p style="margin:14px 0 6px;font-size:52px;font-weight:bold;color:#0f766e;letter-spacing:14px;font-family:monospace;">{otp}</p>
...
def send_support_rating_confirmation(email: str, name: str, rating: int):
    ...
    stars = "⭐" * rating
    html = _wrap(f"""
      <h2 style="color:#0f766e;margin-top:0;font-size:22px;">Thank you for your feedback! 🙏</h2>
      <p style="color:#444;font-size:14px;line-height:1.6;">
        Hi {first}, we truly appreciate you taking the time to rate your experience with our support team.
      </p>
      <div style="background:#f0fdfa;border:2px solid #0f766e;border-radius:12px;padding:24px;margin:20px 0;text-align:center;">
        <p style="margin:0;font-size:36px;">{stars}</p>
        <p style="margin:10px 0 0;color:#0f766e;font-weight:bold;font-size:16px;">You rated us {rating}/5</p>
      </div>
      <p style="color:#444;font-size:14px;line-height:1.6;">
...
def send_support_rating_admin_notify(name: str, email: str, rating: int, category: str, message: str):
    ...
    html = _wrap(f"""
      <h2 style="color:#0f766e;margin-top:0;font-size:22px;">📊 New Support Rating Received</h2>
...
def send_order_cancelled_email(email: str, name: str, order):
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #eee">
      ...
      <div style="padding:32px">
        ...
        <p style="color:#555;font-size:14px">Changed your mind? You can always <a href="{STORE_URL}/products" style="color:#0f766e">shop again</a>.</p>
...
def send_refund_initiated_email(email: str, name: str, order, refund_id: str = ""):
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #eee">
      {_HEADER_HTML}
      <table width="100%" cellpadding="0" cellspacing="0" border="0"
             style="border-collapse:collapse;background:#0f766e;">
      ...
      <div style="padding:32px">
        ...
        <div style="text-align:center;margin:24px 0">
          <a href="{STORE_URL}/orders" style="background:#0f766e;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:bold;font-size:14px">View My Orders</a>
        </div>

        <p style="color:#888;font-size:13px;text-align:center">Questions? <a href="mailto:{SUPPORT_EMAIL}" style="color:#0f766e">{SUPPORT_EMAIL}</a></p>
      </div>
      <table width="100%" cellpadding="0" cellspacing="0" border="0"
             style="border-collapse:collapse;background:#f8f4f0;border-top:1px solid #ede8e3;">
        <tr><td align="center" style="padding:16px 24px;">
        ...
        <table width="100%" style="border-collapse:collapse;font-size:13px;">
          ...
          <tr>
            ...
            <td style="padding:8px 0;text-align:right;vertical-align:top;">
              <a href="mailto:{SUPPORT_EMAIL}" style="color:#0f766e;text-decoration:none;font-weight:bold;">
                {SUPPORT_EMAIL}
              </a>
            </td>
          </tr>
...
_RETURN_STATUS_INFO = {
    ...
    "pickup_scheduled":     ("Pickup Scheduled",            "#0f766e", "Our courier will pick up the item from your address."),
    ...
    "replacement_shipped":  ("Replacement Shipped",         "#0f766e", "Your replacement item is on its way!"),
...
def send_return_request_email(email: str, name: str, order, rr):
    ...
    html = _wrap(f"""
      <h2 style="color:#0f766e;margin-top:0;font-size:22px;">{type_label} Request Received</h2>
      <p style="color:#444;font-size:14px;line-height:1.6;">
...
def send_return_status_email(email: str, name: str, order, rr):
    first = name.split()[0]
    type_label = _RETURN_TYPE_LABEL.get(rr.request_type, rr.request_type.title())
    title, color, msg = _RETURN_STATUS_INFO.get(rr.status, (f"Update: {rr.status}", "#0f766e", ""))
    ...
    <div style="font-family:Arial,sans-serif;max-width:650px;margin:auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #eee">
      ...
      <div style="padding:28px 32px">
        ...
        <table width="100%" style="border-collapse:collapse;margin-bottom:24px">
          <tr>
            ...
            <td style="text-align:right;font-size:13px;color:#555">
              <p style="margin:0 0 4px"><b>Order Number:</b> <span style="color:#0f766e;font-weight:bold">{order.order_number}</span></p>
        ...
        <div style="background:#fdf2f4;border-radius:8px;padding:14px 16px;margin-bottom:20px">
          <p style="margin:0 0 6px;font-size:11px;font-weight:bold;color:#0f766e;text-transform:uppercase;letter-spacing:1px">Shipping Address</p>
        ...
        <table width="100%" style="border-collapse:collapse;margin-bottom:20px">
          ...
          <tr style="background:#fdf2f4">
            <td style="padding:12px 8px;font-size:16px;font-weight:bold;color:#0f766e">Grand Total</td>
            <td style="text-align:right;font-size:16px;font-weight:bold;color:#0f766e;padding:12px 8px">₹{order.total:,.0f}</td>
          </tr>
        </table>
        ...
        <div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:8px;padding:14px 16px;margin-bottom:20px">
          ...
          <table width="100%" style="border-collapse:collapse;font-size:13px">
            ...
            <tr><td style="color:#555;padding:3px 0">Status</td><td style="text-align:right;font-weight:bold;color:{'#16a34a' if order.payment_status == 'paid' else '#d97706' if order.payment_status == 'pending' else '#0f766e'}">{order.payment_status.upper()}</td></tr>
        ...
        <!-- Download button -->
        <div style="text-align:center;margin:24px 0">
          <a href="{STORE_URL}/orders/{order.id}/invoice" style="background:#0f766e;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:bold;font-size:14px">📄 View & Download Invoice</a>
        </div>
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #eee">
      ...
      <div style="padding:32px">
        ...
        <div style="background:#fdf4ff;border:1px solid #e9d5ff;border-radius:10px;padding:16px;margin:20px 0">
          <table width="100%" style="border-collapse:collapse;font-size:14px">
            <tr><td style="color:#555;padding:5px 0">Support Agent</td><td style="text-align:right;font-weight:bold;color:#0f766e">{cs_name}</td></tr>
            {issue_row}
          </table>
        </div>
        <p style="color:#555;text-align:center;font-size:14px">Click below to rate your experience — it only takes 2 seconds:</p>
        <div style="text-align:center;margin:24px 0">
          <a href="{rating_url}" style="background:#0f766e;color:#fff;padding:14px 36px;border-radius:8px;text-decoration:none;font-weight:bold;font-size:16px">⭐ Rate My Experience</a>
        </div>
...
def send_admin_access_email(email: str, name: str):
    ...
    html = _wrap(f"""
      ...
      <!-- What you can do -->
      <div style="background:#f0fdfa;border-radius:12px;padding:20px 24px;margin:20px 0;">
        <p style="margin:0 0 14px;font-size:13px;font-weight:bold;color:#0f766e;text-transform:uppercase;letter-spacing:1px;">What you can do</p>
      ...
      <div style="background:#fdf2f4;border-radius:10px;padding:16px 20px;margin-bottom:20px;">
        <p style="margin:0 0 6px;font-size:13px;font-weight:bold;color:#0f766e;">How to access the Admin Dashboard</p>
        <p style="margin:0;font-size:13px;color:#555;line-height:1.6;">
          Log in to <a href="{STORE_URL}" style="color:#0f766e;">{STORE_URL}</a> with your account.
...
def send_admin_revoked_email(email: str, name: str):
    ...
    html = _wrap(f"""
      ...
      <div style="background:#f0fdfa;border-radius:12px;padding:20px 24px;margin:20px 0;">
        <p style="margin:0 0 10px;font-size:13px;font-weight:bold;color:#0f766e;text-transform:uppercase;letter-spacing:1px;">What this means</p>
        <table style="width:100%;border-collapse:collapse;">
          <tr>
      ...
      <div style="background:#fdf2f4;border-radius:10px;padding:16px 20px;margin-bottom:20px;">
        <p style="margin:0;font-size:13px;color:#666;line-height:1.6;">
          ...
          <a href="mailto:{SUPPORT_EMAIL}" style="color:#0f766e;font-weight:bold;">{SUPPORT_EMAIL}</a>.
        </p>
      </div>
